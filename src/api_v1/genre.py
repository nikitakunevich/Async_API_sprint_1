...
from schemas.genre import GenreDetail
...
async def genres_all(
    ...
    return [GenreDetail(uuid=genre.id, name=genre.name, filmworks=genre.filmworks)
            for genre in genres]
...
async def genre_detail(
    ...
    genre = await genre_service.get_by_id(str(genre_id), 'genres')
    if not genre:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='genre not found')
    return GenreDetail(uuid=genre.id, name=genre.name, filmworks=genre.filmworks)
